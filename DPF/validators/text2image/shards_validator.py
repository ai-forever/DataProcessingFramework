...
class ShardsValidator(T2IValidator):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            validate_captions=validate_captions,
        )

        self.validate_tars = validate_tars

    def _validate_tar(
        self, csv_path: str, df: pd.DataFrame, errors: dict, error2count: Dict[str, int]
    ):
        ...
        images_in_csv_but_not_in_tar = image_names_in_csv_set.difference(
            image_names_in_tar_set
        )
        images_in_tar_but_not_in_csv = image_names_in_tar_set.difference(
            image_names_in_csv_set
        )
        ...
        duplicated_images_in_tar = np.unique(
            get_duplicated_elements(image_names_in_tar)
        )
    ...
    def validate(
        self, dataset_dir: str, processes: int = 1
        ...
        assert len(files_tar) == len(files_csv) and set(files_csv) == set(
            files_csv_renamed
        ), "Every .tar file should have .csv file with same filename"

        datas = process_map(
            self.validate_shard, files_csv, max_workers=processes, chunksize=1
        )
        csv2errors = {}
        error2count_all = {}
        all_ok = True
