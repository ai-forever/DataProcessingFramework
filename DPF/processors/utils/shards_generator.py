...
class ShardsGenerator:
    def __init__(self, df, save_path, processes=8, images_per_tar=1000, force=False, rename_images=False,
                 save_csv=True, imagename_column="image_name", columns_to_add=[]):
        self.df = df
    ...
    def _flush_chunk(self, samples, shard_path, shard_number):
        ...
        for c, data in enumerate(samples):
            ...
            if self.rename_images:
                image_name = str(self.images_per_tar*shard_number+c) + get_file_extension(image_path)
            else: 
    ...
    def run(self):
        ...
        assert self.force or len(os.listdir(self.save_path)) == 0, f"Directory is not empty. Set force=True to ignore this"
        ...
        for chunk_id, (a, b) in tqdm(enumerate(zip(
            ...
            np.arange(self.images_per_tar, self.df.shape[0]+self.images_per_tar, self.images_per_tar),
        ))):
            chunk = self.df[a:b]
            ...
            assert is_list_has_no_duplicates(image_names), "Image names are duplicated, set rename_images=True"
