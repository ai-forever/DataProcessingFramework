...
class FaceFocusFilter(ImageFilter):
    ...
    def __init__(
        ...
        self.face_focus_threshold = face_focus_threshold
        self.num_workers = workers
        self.batch_size = batch_size

    @property
    def result_columns(self) -> list[str]:
        return ["face_detected", "face_in_focus", "face_focus_measure"]
    ...
    def process_batch(self, batch: list[Any]) -> dict[str, list[Any]]:
        ...
        for key, image in batch:
            face_info = process_image(image, face_focus_threshold=self.face_focus_threshold)
            if face_info:
...
def process_image(image, face_focus_threshold=2000.0):
    ...
    if len(filtered_faces) == 1 and face['confidence'] > 0.5:
        ...
        if face['facial_area']['left_eye'] is not None and face['facial_area']['right_eye'] is not None:
            ...
            face_in_focus = face_focus_measure > face_focus_threshold
