...
def load_pretrained_model(model_path, load_8bit=False, load_4bit=False, device='cuda:0', device_map="auto"):
    model = LEGOLlamaForCausalLM.from_pretrained(model_path,torch_dtype=torch.bfloat16).to(device)
    ...
    image_tower = CLIPVisionModel.from_pretrained('openai/clip-vit-large-patch14-336',torch_dtype=torch.bfloat16,low_cpu_mem_usage=True).to(device)
