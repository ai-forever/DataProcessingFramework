...
class CLIPFilter(T2IFilter):
    ...
    def __init__(
        ...
        self.clip_model, self.clip_processor = clip.load(clip_version, device=self.device,
                                                         download_root=self.weights_folder)
    ...
    def process_batch(self, batch) -> dict:
        ...
        with torch.no_grad():
            ...
            for template in self.templates:
                texts = clip.tokenize([template.format(class_label)
                                       for class_label in batch_labels], truncate=True)
